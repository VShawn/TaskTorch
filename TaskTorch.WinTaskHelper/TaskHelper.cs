...
namespace TaskTorch.WinTaskHelper
{
    public static class TaskHelper
    {
        ...
        public static Task GetTmpTask(string taskName)
        {
            using (var ts = new TaskService())
            {
            ...
            string userName = null, string password = null) where T : Trigger
        {
            ...
            using (var ts = new TaskService())
            {
                if (string.IsNullOrEmpty(userName) || string.IsNullOrEmpty(password))
                    return ts.AddTask(taskName, tigger, new ExecAction(runnerPath, scriptName, System.Environment.CurrentDirectory), null, null, TaskLogonType.InteractiveToken,
                        nameof(TaskTorch) + ":" + taskDescription);
                else
                    return ts.AddTask(taskName, tigger, new ExecAction(runnerPath, scriptName, System.Environment.CurrentDirectory), userName, password,
                        TaskLogonType.InteractiveTokenOrPassword, nameof(TaskTorch) + ":" + taskDescription);
            }
        }
        public static Task AddTmpTask(string taskName, string taskDescription, string scriptName, TriggerCollection triggers,
